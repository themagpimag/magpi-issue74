...
// also because Bob is so importont to us we name him and give him a pointer in the class defintion now
	...
	MyBob = new Bob();
	MyBob->TheOGLInstance = MyOGLES;   // Bob's render system needs to know where the OGELS class is, since it has info on the basic shader
...
// we can go through a vector the same way we can an array to update the objects we put in the vector
	...
	// lets use it in 2 different ways
		char TempBuffer[130];
		if (HowManyStars)
			std::sprintf(TempBuffer, "We have %i stars to collect and %i", HowManyStars, this->MyBob->BaseAnim);
		else
			 if (TimerCountDown == 0)
			{
